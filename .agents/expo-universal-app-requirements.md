...
Latest local PP-OCR model migration:

- Web and mobile now target PP-OCR v6 Small. Web selects the official `PP-OCRv6_small_det` and `PP-OCRv6_small_rec` assets explicitly; mobile uses the fixed `ppocr-v6-small` ID and native ONNX model configuration.
- The published mobile pack is 31,277,677 bytes and contains dynamic-shape detector/recognizer ONNX files, inference configs, an 18,708-entry dictionary, and Apache-2.0 license metadata. The recognizer exposes 18,710 output classes, matching blank plus dictionary plus space decoding.
- The fixed R2 manifest and all six runtime files pass the 125-check remote asset audit. A Release iOS simulator downloaded the production packs and executed PP-OCR v6 Small through ONNX Runtime in 33 ms with one detected line. PP-OCR v5 Mobile was then removed from R2; signed-iPhone evidence remains pending.
...
| App shell and background | Centered floating card on a plain indigo/purple/pink gradient with no pattern | Full-screen native sheet layout (top corner radius, flush bottom) on a web-aligned gradient; the background keeps a softened SVG star/wave pattern (opacity 0.04 light / 0.05 dark) as a deliberate platform exception | P1 | Implemented; theme tokens and shadows now align with the web palette (`#eef2ff -> #faf5ff -> #fdf2f8`, indigo `#6366f1`, flat `0 4px 0 rgba(0,0,0,0.1)` shadows); visual QA on device pending |
...
- Local models: Settings shows fixed Whisper Base, SenseVoice Small, and PP-OCR v6 Small download/update/delete controls. Mobile exposes no local path, runtime-assets URL, or arbitrary manifest URL. Models download only from `assets.tabitomo.alkinum.io`; file URLs are same-origin enforced, byte/SHA-256 verified, staged, native-load validated, and atomically activated. Sessions unload before deletion. Model binaries and download state never enter iCloud sync or `.ttconfig` export.
...
Status: fixed R2 model distribution and native download storage are implemented. The `tabitomo-assets` R2 bucket is bound to `assets.tabitomo.alkinum.io` with active TLS 1.2; verified Whisper Base int8, SenseVoice Small int8, and PP-OCR v6 Small assets are published with per-file SHA-256 manifests. The current three-model Release simulator smoke downloads and executes each native runtime successfully; PP-OCR v5 Mobile has been removed from R2. Mobile Settings downloads fixed models without path or arbitrary manifest input. Real-device download/storage and inference QA remain pending.
...
- 2026-09-08 UI refinement: removed the Japanese brand suffix and tilted empty-state tile on Web and iPhone; quiet segmented modes, grouped horizontal language controls, semantic neutral text and softer shared indigo surfaces. Native input tools retain touch targets, Clear moves to the source heading, and a labeled primary action replaces the checkmark. Phone provider selection uses an expandable list while Web retains select; preset and credential semantics are unchanged. 320px uses shorter empty-state spacing; up to 430px uses phone panel padding. See `.agents/design-and-model-revision.zh-CN.md` for this iteration and verification.
